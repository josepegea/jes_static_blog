...
Created a patch and submitted a PR to fix the issue that was merged
into master.

<%= hang_left_icon('github') %>
**PR**: <https://github.com/EmpaticoOrg/scenic-mysql_adapter/pull/5>

### _Active Support**2019**_ Patch to improve ActiveSupport::Duration

When pretty-printing `ActiveSupport::Duration` objects out of
additions, the results are not normalized (i.e. "3 minutes and 70
seconds").

Created a patch to add a `normalize` method and submitted a PR to
